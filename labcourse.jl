...
``t_{\text{max}}`` = $(@bind tmax Slider(0.0u"ns":1.0u"ns":100.0u"ns", default=10.0u"ns", show_value=true))
...
begin
	rs_eu = euler(eom, r0, par, tmax, dt)
	rs_rk = runge_kutta_4(eom, r0, par, tmax, dt)
	rs_pc = predictor_corrector(eom, r0, par, tmax, dt)
end

# ╔═╡ 96002c68-50b6-409b-ad31-c2a7967414b0
[r[1] for r in rs_eu] ### E.g. to access the array of time steps

# ╔═╡ 239ddee7-8f0f-4dae-a12a-e895c5c3d5ce
[r[5] for r in rs_eu]

# ╔═╡ 5a3cdea5-5140-43cb-a609-ae96161967ce
### Plot trajectory of the particle: x(t), y(t), z(t), (x,y)(t), x,y(z)
function plotresults_xt(rs)
	t = [r[1] for r in rs]
	x = [r[2] for r in rs]
	return plot(t, x, xlabel="Time", ylabel="x-Position", legend=false)
end
### Check that energy conserves, plot En(t)
### Repeat this for all three methods and compare trajectories.

# ╔═╡ 665b19f9-0545-4767-962f-c1a180e66863
plotresults_xt(rs_eu)

# ╔═╡ ba7f3e06-90c9-4a3f-b79a-28f4085536fa
function plotresults_yt(rs)
	t = [r[1] for r in rs]
	y = [r[3] for r in rs]
	return plot(t, y, xlabel="Time", ylabel="y-Position", legend=false)
end

# ╔═╡ 58fa1997-0b7b-41f7-846f-2245aa0f9564
function plotresults_xy(rs)
	x = [r[2] for r in rs]
	y = [r[3] for r in rs]
	return plot(x, y, xlabel="x-Position", ylabel="y-Position", legend=false)
end

# ╔═╡ ae69625e-c239-4647-9599-1435d41e3de6
function plotresults_NRG(rs)
	t = [r[1] for r in rs]
	NRG = [r[5] for r in rs]
	
	return plot(t, NRG, xlabel="Time", ylabel="Energy", legend=false)
end

# ╔═╡ 65aa8b9f-cc2f-471d-a52f-87960d38205c
function plotres3D(rs)
	x = [r[2] for r in rs]
	y = [r[3] for r in rs]
	z = [r[4] for r in rs]
	return plot(x, y, z)
...
# ╔═╡ 8ed52e21-cbc3-4125-81b8-a468b5a0d896
begin
	p1=plotresults_xt(rs_eu)
	p2=plotresults_yt(rs_eu)
	plot(p1,p2)
end

# ╔═╡ d0635f92-0657-43ab-91b7-92f2219e8b57
plotresults_NRG(rs_eu)

# ╔═╡ f49d3ad1-e440-49b1-83a4-0cb89a484f43
begin
	t = [r[1] for r in rs_eu]
	
	x1 = [r[2] for r in rs_eu]
	y1 = [r[3] for r in rs_eu]
	
	x2 = [r[2] for r in rs_pc]
	y2 = [r[3] for r in rs_pc]
	
	x3 = [r[2] for r in rs_rk]
	y3 = [r[3] for r in rs_rk]

	NRG1 = [r[5] for r in rs_eu]
	NRG2 = [r[5] for r in rs_pc]
	NRG3 = [r[5] for r in rs_rk]

	plot(t, [x1 x2 x3], label=["Euler" "PC" "RK4"], lw=[2 2 2],ls=[:solid :dash :dot])
	title!("x-Positions at dt=$dt")
end

# ╔═╡ 9ffa7e79-beff-4c14-b64b-ee0eb746de0b
begin
	plot(t, [y1 y2 y3], label=["Euler" "PC" "RK4"], lw=[2 2 2],ls=[:solid :dash :dot])
	title!("y-Positions at dt=$dt")
end

# ╔═╡ 56e5d27e-99fc-44ab-ad47-b3b91ac27327
begin
	plot(x1,y1, lw=2, ls=:solid, label="Euler")
	plot!(x2,y2, lw=2, ls=:dash, label="PC")
	plot!(x3,y3, lw=2, ls=:dot, label="RK4")
	title!("Simulated trajectory at dt=$dt")
end

# ╔═╡ 85b62a7a-ba8e-4d23-8ed4-2c5ab9ef7ef4
begin
	plot(t, [NRG1 NRG2 NRG3], label=["Euler" "PC" "RK4"], lw=[2 2 2],ls=[:solid :dash :dot])
	ylims!(1.3,1.5)
	title!("Energy of different methods dt=$dt")
end

# ╔═╡ 2729104d-d9c6-4949-ba66-31bbb45866c8
plotres3D(rs_eu)
...
begin
	cdf_values = cumsum(bin_contents)/sum(bin_contents) 
...
begin
	...
	simpdf = histogram(bin_positions, weights=accepted_samples_hm, bins=62, xlabel="Mass (MeV/c²)", ylabel="Candidates", title="Hit-or-Miss N=$num_samples", legend=false)
end
...
begin
	...
	### Plot histogram with the simulated sample
	siminv = histogram(bin_positions, weights=accepted_samples_inv, bins=62, xlabel="Mass (MeV/c²)", ylabel="Candidates", title="Inverse Transform Method, N=$num_samples", legend=false)
end
...
function chi2(sample, data)
    ...
    chi2_values = (data/sum(data) .- sample/num_samples).^2
		### Implement calculation of chi2 values (Hint: use broadcasting or implement loop)
...
# ╔═╡ c536e6c6-2955-4cba-ae13-cc2b6415f416
function hit_and_miss_sampling_improved(bin_contents, bin_positions, num_samples)	
	max_bin_content = maximum(bin_contents)
	accepted_samples_hm = zeros(lastindex(bin_positions))
	for i in 1:num_samples
		random_x = trunc(Int, rand() * (lastindex(bin_positions)-1) + 1)
		random_y = rand() * (max_bin_content * exp(-0.5 * ((random_x - 3100)/10)^2)+(7000))
...
# ╔═╡ 6d4a8db9-0d3f-41d2-b98d-c723cffded7c
begin
	accepted_samples_hmimproved = hit_and_miss_sampling_improved(bin_contents, bin_positions, num_samples)
	### Plot histogram with the simulated sample
	simhmimproved = histogram(bin_positions, weights=accepted_samples_hmimproved, bins=62, xlabel="Mass (MeV/c²)", ylabel="Candidates", title="Inverse Transform Method, N=$num_samples", legend=false)
end

# ╔═╡ b707a913-e36f-4daf-aa6c-bb2600fdd4f7
chi2_hmimproved = chi2(accepted_samples_hmimproved, bin_contents)

# ╔═╡ 8007f1e3-a738-40da-8379-0c6b555610ae
eff_hmimproved = efficiency(accepted_samples_hmimproved, num_samples)

# ╔═╡ 421a22fb-5e86-4431-93d5-42e939b1627f
begin
	improvedpdf= [8000 * exp(-0.5 * ((x - 3100)/10)^2)+(-.1*x+7000) for x in bin_positions]
	plot(bin_positions,improvedpdf, lw=2)
	histogram!(bin_positions, weights=bin_contents, bins=62, xlabel="Mass (MeV/c²)", ylabel="Candidates", title="Histogram of Resonance Data", legend=false)
end
...
begin
    ...
    for M in M_mumu
		...
		p1abs = sqrt(p1[1]^2/c_1^2 - m_mu^2*c_1^2)
		push!(pmu_minus, p1abs)
		p2abs = sqrt(p2[1]^2/c_1^2 - m_mu^2*c_1^2)
		push!(pmu_plus, p2abs)
		mass = jpsi_from_mumu(p1, p2)
		push!(M_mumu_sim, mass)
		
        r0_minus = [tmu0, xmu0, ymu0, zmu0, p1[1], p1[2], p1[3], p1[4]]
        r0_plus = [tmu0, xmu0, ymu0, zmu0, p2[1], p2[2], p2[3], p2[4]]

        rs_minus = runge_kutta_4(eom, r0_minus, par_minus, tmumax, dtmu)
        rs_plus = runge_kutta_4(eom, r0_plus, par_plus, tmumax, dtmu)

        rlast_minus = rs_minus[length(rs_minus)]
        rlast_plus = rs_plus[length(rs_plus)]

        Rminus, pminus = momentum_from_circle(r0_minus, rlast_minus, -q, B_exp)
		Rplus, pplus = momentum_from_circle(r0_plus, rlast_plus, q, B_exp)
		push!(pmu_minus_sim, pminus)
		push!(pmu_plus_sim, pplus)
		
    end
end

# ╔═╡ 97ee3366-4afb-4d80-b301-3316f90aceb8
M_mumu

# ╔═╡ 5742aebf-06f8-4369-a19c-4f739d6787ec
M_mumu_sim

# ╔═╡ f9c934d1-707c-4e56-be8f-21d667e4bfae
pmu_minus

# ╔═╡ f2180b17-de94-4f97-b5e6-61e530ad4b44
pmu_minus_sim_converted=[uconvert(u"GeV/c", p) for p in pmu_minus_sim]

# ╔═╡ 192b8c39-f2a0-475a-88f4-34ca0781974a
delta_p=(pmu_minus.-pmu_minus_sim_converted)./pmu_minus

# ╔═╡ faa48897-e540-4bf8-ab1f-3f90a15a60d7
mean(delta_p)

# ╔═╡ 9e652c3c-ce84-456e-a552-efc61299c1a9
std(delta_p)
